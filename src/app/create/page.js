...
const create = () => {
    const { pairingData, connect, createJar } = useHedara();
    ...
    const handleApprovers = (value) => {
        if (!selectedApprovers.some((approve) => approve.name === value)) {
            const newApprovers = names.find(
                (approve) => approve.name === value
            );
            ...
            <div className="grid grid-cols-11 items-start justify-start ml-14">
                <Form onSubmit={handleSubmit}>
                    ...
                    <span className="col-span-4 mt-2 text-[12px] font-inter font-medium text-b3 text-justify">
                        Assets stored inside the Jar need to approve by
                        majorities in order to be payout to the recipient. Add
                        the the approvers from your approvers list and they will
                        need to use their wallet to approve any transactions to
                        the recipient.
                    </span>
